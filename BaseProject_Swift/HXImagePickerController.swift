...
class HXImagePickerController: UINavigationController {
    ...
    /// 最大选择图片数
    @objc var maxSelectCount: Int = 9

    /// 主题颜色
    @objc var mainTintColor: UIColor = .red
    
    /// 选择媒体类型
    var mediaTypes: [PHAssetMediaType] = [.video, .image]
    
    /// 是否包含相册列表，否则只有相机胶卷
    @objc var hasAlbumList: Bool = true
    
    /// 回调代理
    @objc weak var hxip_delegate: HXImagePickerControllerDelegate?
    ...
    @objc init(hasAlbumList: Bool = true) {
